...
class cluster {
	...
	std::vector<std::vector<int> > GlobalConnectVec; // making a copy!! == Odist_Array
...
public:
	...
	cluster(const std::vector<std::vector<int> > &GlobalConnect,
			std::vector<int> &Phase, const float boxcentre) :
			GlobalConnectVec(GlobalConnect), OxygenPhase(Phase), BoxCentre(
					boxcentre) {
	}
	void connectivity();

	void size(const float*, bool);
