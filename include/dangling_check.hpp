...
public:
	...
	void danglingOx(const int, const float*, const float*, const float*, bool,
			const float);
	std::vector<std::vector<int> > getDangVec() const;// returns danglinglist vector
	std::vector<std::vector<int> > getDangTopVec() const {
		return DanglingTop;
	}
	;     	// returns DanglingTop vector        
	std::vector<std::vector<int> > getDangBotVec() const {
		return DanglingBot;
	}
	;     	// returns DanglingBot vector
