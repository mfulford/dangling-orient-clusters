...
public:
	...
	float Initialise(bool, bool, int, const float*);
	void Average(const std::vector<float> &,
			const std::vector<std::vector<int> > &);
	void Average(const std::vector<std::vector<float> > &,
			const std::vector<std::vector<int> > &);

	void TimeAverage(bool, const std::vector<int> &);
	void TimeAverage(int, bool, bool, const std::vector<int> &);
	void InPlaneTimeAv(int, bool, bool, const std::vector<std::vector<int> > &);
	void InPlaneDensity(int, const std::vector<std::vector<int> > &,
			const std::vector<std::vector<float> > &,
			const std::vector<float> &, bool, const float*, const float*);

	void NormaliseHeatmaps();
	void NormaliseHeatmaps_nQLL();

	const std::vector<float> getTimeBinAvVec(bool); // {return TimeBinAvVec;};
	const std::vector<std::vector<float> > getTimeAv_nQLLVec(bool); //{return TimeAv_nQLL;};
	const int* getTimeAv_countnQLL() const {
		return count_nQLL;
	}
	;
	...
	const std::vector<
			std::vector<std::vector<std::vector<std::vector<float> > > > > getAvPlane_nQLL();
	const std::vector<std::vector<std::vector<std::vector<float> > > > getOrientAvPlane_nQLL();

	std::vector<float> const getDensityVec() {
		return densityVec;
	}
	;

	void orient_slice(int, const std::vector<std::vector<int> > &,
