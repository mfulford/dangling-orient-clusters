...
#include "dataCell_pbc.hpp"     // executes pbc conditions and provides cell data
...
void orientation::orientationOH(const int num_ox, const int frame,
		const float* x, const float* y, const float* z, bool IsPrism,
		const float box_midpoint) {
	...
	for (int i = 0; i < num_ox; i++) {
		...
		// Mid point between hydrogens following pbc
		vector<float> h_mid;
		h_mid = cellf.h_midpoint(x[i * 3 + 1], y[i * 3 + 1], z[i * 3 + 1],
				x[i * 3 + 2], y[i * 3 + 2], z[i * 3 + 2]); 

		// h_mid --> O vector is the molecular dipole	
		xx = x[i * 3] - h_mid[0];
		yy = y[i * 3] - h_mid[1];
		zz = z[i * 3] - h_mid[2];
		...
		OH_vect = {xx, yy, zz};
		mag_OH = sqrt(
				inner_product(OH_vect.begin(), OH_vect.end(), OH_vect.begin(),
						0.0));
		xx = xx / mag_OH;
		yy = yy / mag_OH;
		zz = zz / mag_OH;
		...
		mag_H = sqrt(
				inner_product(H_vect.begin(), H_vect.end(), H_vect.begin(),
						0.0));
		xx_H = xx_H / mag_H;
		yy_H = yy_H / mag_H;
		zz_H = zz_H / mag_H;

		vector<float> alpha_x(3);
		...
		// N = cross product of dipole and "z" N = dipole x z. Perpendicular to dipole and "z" axis
		// Basal "z"=y=(0,1,0). Prism "z"=x=(1,0,0)
		// Gamma is angle of H-H with N
		N_vect = {Nx, Ny, Nz}; 			
		mag_N = sqrt(
				inner_product(N_vect.begin(), N_vect.end(), N_vect.begin(),
						0.0));
		Nx = Nx / mag_N;
		Ny = Ny / mag_N;
		Nz = Nz / mag_N;
		...
		// Check for rounding errors pushing values above 1 or below -1
...
void orientation::average(const vector<vector<int> > &DanglingTop,
	...
	cout << "#ORIENTDANG Top Layer Average: " << avTop << " Bottom Layer: "
			<< avBot << endl;

}

// Calculate average dipole theta of QLL molecules: 
void orientation::average(const vector<int> &QLLTop,
		const vector<int> &QLLBot) {
	...
	cout << "#ORIENTQLL Top Layer Average: " << avTop << " Bottom Layer: "
			<< avBot << endl;
