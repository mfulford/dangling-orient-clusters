...
void dangling::danglingOx(const int frame, const float* x, const float* y, const float* z, vector<bool>  IcePhase, const float box_midpoint) {
	...
	// vector with num_ox rows with ever element =0
	vector<int> neighlist(num_ox,0);
	...
	for (int i=0;i<num_ox;i++){
		...
		for (int ii=i+1;ii<num_ox;ii++){
	  	...
	  	// if i has no neighbours then it is dangling:
	  	if (Odist_Array[i].empty() ) {	
			cout << "#DANGLING Dangling(No Neighbours), i: " << i << " Number Neighbours: " << Odist_Array[i].size() <<  endl;
			dang1.push_back(i); 				
			dang2.push_back(i);				
		}
		// if not, it still may be dangling depending on angles:
			...
			for (int n=0;n<Odist_Array[i].size();n++) {
				...
				OO = { xxOO, yyOO, zzOO};               
				mag_OO = sqrt( inner_product( OO.begin(), OO.end(), OO.begin(),0.0));     
				...
				for (int j=0;j<2;j++){
                    ...
                    OH = { xxOH, yyOH, zzOH};
					mag_OH = sqrt( inner_product( OH.begin(), OH.end(), OH.begin(),0.0));                         		
					
					// dot product of OH and OO vectors:
					dot_OO_OH = inner_product(OH.begin(), OH.end(), OO.begin(), 0.0);	
					cosAng = dot_OO_OH/(mag_OO * mag_OH);
					
					// check if angle greater than 30:
					if (cosAng < cut_ang) {count_ang[j] = count_ang[j] +1;} 			 				
	...
	//loop over OH1 and OH2. DanglingTop/Bot are each 2d vectors for each H in a molecule. Must distinguish between both OH bonds for when we calculate orientation 
	for (int H=0;H<2;H++) {

		/* loop over dangling OH(#H) and determine which QLL belongs to using midpoint of system: */
		for (int i=0;i<danglinglist[H].size();i++) {
			/* if X-axis: Secondary Prism */
			if (IcePhase[0] == true) {
			...
			/* else if Z-axis: Basal */
        	else if (IcePhase[2] == true ) {
        		if (z[danglinglist[H][i]*3] >= box_midpoint) {
        			dangtopOH.push_back(danglinglist[H][i]);
        		}
        		else {
        			dangbotOH.push_back(danglinglist[H][i]);
