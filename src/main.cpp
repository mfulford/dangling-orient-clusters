...
// Compile: g++ -std=c++11 *.cpp -o Executable

// valgrind --leak-check=yes ./Executable [inputs]
// valgrind --tool=memcheck --db-attach=yes ./Executable [inputs]
...
int main(int argc, char* argv[]){
	...
	char answer_qll;
	bool IsConstQLL;
	cout << "Type Y if this simulation is constant QLL thickness (ie normal MD)" << endl;
	cout << "Otherwise type N if this simulation is metadynamics or MD with dynamic QLL Size" << endl;
	cout << "NB: If option \"N\", code will only work if 1 surface QLL size is dynamic (not both)" << endl;
	cin  >> answer_qll;
	...
	/* determine Ice phase
	 * IcePhase[0] = true: X-axis Secondary Prism
	 * IcePhase[1] = true: Y-axis Prism
	 * IcePhase[2] = true: Z-axis Basal  */
	pbc_check phase_check;
	vector<bool> IcePhase = phase_check.Phase();

	/* Istance of q3 file and open: */
	q3_r q3f(argv[3],dcdf.getNATOM()/3, q3threshold);

	/* instance of data output */
	Output outf("dang_time.txt", "orient_frame.txt","cluster_time.txt", "noxygens_time.txt",
			"interface_time.txt","cluster_Bulk_tcl.txt", "q3_tcl.txt", "qll_tcl.txt",
			dcdf.getNATOM(),dcdf.getNFILE(), IcePhase);
	...
	float binwidthPlane1 = 0.0;
	float binwidthPlane2 = 0.0;
	...
	for(int i=0;i<dcdf.getNFILE();i++){
		...
		/* Set coord_norm=coordinate normal to surface */
		static const float* coord_norm;
		static const float* coord_inplane1;
		static const float* coord_inplane2;
		...
		else if (IcePhase[1] == true) {
			coord_norm=y;
			coord_inplane1=x;
			coord_inplane2=z;
		}
		else if (IcePhase[2] == true) {
			coord_norm=z;
			coord_inplane1=x;
			coord_inplane2=y;
		}
		...
		if (i == 0 ) { box_midpoint= phase_check.box_centre(dcdf.getNATOM(),coord_norm);}

		clock_t t_readData = clock();

		// --------------------------------------------------------------------------------------------------------
		// ----------------- CALCULATE DANGLING and ORIENTATION stats: --------------------------------------------
		dangframe.danglingOx(i,x,y,z, IcePhase, box_midpoint);
		int numDangTop = dangframe.getDangTopVec()[0].size() + dangframe.getDangTopVec()[1].size();
		int numDangBot = dangframe.getDangBotVec()[0].size() + dangframe.getDangBotVec()[1].size();
		orient_frame.orientationOH(dcdf.getNATOM()/3,i,x,y,z, IcePhase, box_midpoint);
		// ----------------- Calculated dangling + orientation -----------------------------------------------------

		clock_t t1_statsDangOrient = clock();

		// IMPORT q3 data for current frame and determine liquid and ice molecules:
		q3f.q3_frame();	
		...
		cluster WaterCluster(dangframe.getOxArrayVec(),q3f.liquid,box_midpoint);
		// connectivity matrix of liquid Ox
		WaterCluster.connectivity();
		WaterCluster.size(coord_norm, IcePhase);
		// ----------------------------------------------------------------------------------------------------------
		...
		cluster BulkIceCluster(dangframe.getOxArrayVec(),q3f.crystal,box_midpoint);
		BulkIceCluster.connectivity();
		BulkIceCluster.size(coord_norm, IcePhase);
		// ----------------------------------------------------------------------------------------------------------

		clock_t t3_IceClust = clock();

		// 3) QLL/Ice interface -------------------------------------------------------
		//Determine the QLL interface based on the cluster coordinates
		...
		cout << "####QLL-INTERFACE (cluster) TOP: " << topInterface << " BOT: " << botInterface
				<< " Bulk: " << maxCoordIC << " " << minCoordIC << " Water " << minCoordWC_top << " " << maxCoordWC_bot <<   endl;
		...
		// 4) QLL Ice cluster ---------------------------------------------------------------------------------------
		cluster IceCluster(dangframe.getOxArrayVec(),q3f.qllCrystalOx,box_midpoint);
		IceCluster.connectivity();
		IceCluster.size(coord_norm, IcePhase);
		// ----------------------------------------------------------------------------------------------------------
		...
		cout << "#MAIN Water-like Cluster Size. Top Surface: " << sizeWC_top << " Bottom Surface: " << sizeWC_bot << endl;
		cout << "#MAIN QLL-Ice-like Cluster Size. Top Surface: " << sizeQLLIC_top << " Bottom Surface: " << sizeQLLIC_bot << endl;
		...
		Density.Initialise(IsFirstFrame, dcdf.getNATOM()/3, coord_norm);
		...
		if (IsConstQLL) {
			...
			Q3binAv.TimeAverage(q3f.getqllTopVec().size(),IsFirstFrame,IcePhase,  Density.NumElementsVec_q3);
			Density.TimeAverage(q3f.getqllTopVec().size(),IsFirstFrame,IcePhase,  Density.NumElementsVec);
		}
		// ----------------------------------------------------------------------------------------------------------
		...
		Averages_q3Orient.average(dangframe.getDangTopVec(), dangframe.getDangBotVec(),orient_frame.getOrient(),q3f.q3or);
		...
		Averages_q3Orient.average(q3f.getqllTopVec(), q3f.getqllBotVec(),orient_frame.getOrient(),q3f.q3or);
		...
		if (IsConstQLL) {OrientbinAv.TimeAverage(IsFirstFrame, Density.NumElementsVec);}
		else {	OrientbinAv.TimeAverage(q3f.getqllTopVec().size(),IsFirstFrame,IcePhase,  Density.NumElementsVec);}
		// ----------------------------------------------------------------------------------------------------------
		...
		Density.InPlaneDensity(dcdf.getNATOM()/3, Density.binVec_short, orient_frame.getOrient(),q3f.q3or, coord_inplane1, coord_inplane2);
		Density.InPlaneTimeAv(q3f.getqllTopVec().size(),IsConstQLL, IsFirstFrame, Density.binVec_short); 

		// Calculate probability distribution of orientation in each slice:
		Density.orient_slice(q3f.getqllTopVec().size(), Density.binVec_short, orient_frame.getOrient(), coord_inplane1, coord_inplane2, IsConstQLL, IsFirstFrame);

		binwidthPlane2      = Density.binPlane_data[0] + binwidthPlane2 ;
		binwidthPlane1  = Density.binPlane_data[1] + binwidthPlane1;
		...
		outf.output_dang(i+1, numDangTop, numDangBot, OrientDangTop, OrientDangBot, Q3DangTop, Q3DangBot);
		outf.output_orient(i+1,orient_frame.getOrient());
		outf.output_cluster(i+1, sizeWC_top, sizeWC_bot, sizeQLLIC_top, sizeQLLIC_bot);
		outf.output_noxygens(i+1,q3f.liquid.size(), q3f.crystal.size(), q3f.getqllTopVec().size(), q3f.getqllBotVec().size(), OrientQLLTop, OrientQLLBot, Q3QLLTop, Q3QLLBot);
		outf.interface(i+1,topInterface,botInterface,topSurfaceSize,botSurfaceSize);
		outf.cluster_visual(BulkIceCluster.clusterId,WaterCluster.clusterId, idGlobalIC, idGlobalWC_top, idGlobalWC_bot);
		...
		cout << "####TIMINGS: 1) Dangling Orientation Stats: " << t1 << "    2) WaterCluster: " <<   t2 << "    3) IceCluster: " << t3 << endl;
		cout << "####TIMINGS: 4) QLL Cluster: " << t4 << "    5) Q3 & Density Profile: " << t5 << "    6) Average Orient & Q3 (QLL/Dang): " << t6 << endl;
		cout << "####TIMINGS: 7) Orient Profile: " << t7 << "    8) InPlane : " << t8 << "   9) Output: " << t9 <<   "  TOTAL: " << total << endl;
		cout << " " << endl;
		// ----------------------------------------------------------------------------------------------------------
		// ----------------- COMPLETED TIMINGS ----------------------------------------------------------------------
	...
	binwidthPlane2 = binwidthPlane2/count_planebin;
	binwidthPlane1 = binwidthPlane1/count_planebin;

	float b_width = Density.binDataArray[2];
	float b_width_q3 = Density.binDataArray[4]; //check this
	Density.orient_sliceSUM(IsConstQLL);

	if (IsConstQLL) {
		Density.NormaliseHeatmaps();
		bool IsDensity = false;
		outf.AverageTimeProfile("ProfileOrient.txt",OrientbinAv.getTimeBinAvVec(IsDensity), b_width);
		outf.AverageTimeProfile("ProfileQ3.txt",Q3binAv.getTimeBinAvVec(IsDensity), b_width_q3);
		IsDensity = true;
		outf.AverageTimeProfile("ProfileDensity.txt", Density.getTimeBinAvVec(IsDensity), b_width);

		outf.InPlane("Heatmap_OrientationThetaGamma.txt", "Heatmap_Density.txt", "Heatmap_Q3.txt", "Heatmap_DipoleTheta.txt",
				Density.getAvOrientPlaneVec(), Density.getAvPlaneVec(), binwidthPlane2, binwidthPlane1);
		...
		bool IsDensity = false;
		outf.AverageTimeProfile("ProfileOrient.txt", OrientbinAv.getTimeAv_nQLLVec(IsDensity), OrientbinAv.getTimeAv_countnQLL(), b_width);
		outf.AverageTimeProfile("ProfileQ3.txt", Q3binAv.getTimeAv_nQLLVec(IsDensity), Q3binAv.getTimeAv_countnQLL(), b_width_q3);
		IsDensity = true;
		outf.AverageTimeProfile("ProfileDensity.txt", Density.getTimeAv_nQLLVec(IsDensity), Density.getTimeAv_countnQLL(), b_width);

		outf.InPlane("Heatmap_OrientationThetaGamma.txt", "Heatmap_Density.txt", "Heatmap_Q3.txt", "Heatmap_DipoleTheta.txt",
				Density.getOrientAvPlane_nQLL(), Density.getAvPlane_nQLL(), Density.getTimeAv_countnQLL(), binwidthPlane2, binwidthPlane1);
		outf.orientSlice("ProfileSlice_dipoleTheta.txt", Density.OrientSlice_nqll, Density.getTimeAv_countnQLL());
