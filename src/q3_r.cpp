...
void q3_r::q3_frame(){
	...
	string q3_str;
	float q3_float; //ND this isnt a float!!
	...
	for (int i=0;i<num_ox;i++) {		
		...
		else { q3_float= stof(q3_str);} 

		q3or.push_back(q3_float);

		if (q3f.eof()) { cout << "### End of q3 file " << i << endl;}		

		if (q3or[i] < threshold ) {		// check if liquid-like
			liquid.push_back(i);		// liquid vector contains id of liquid-like Ox
		}
		else { crystal.push_back(i);}
	}

	cout << "#Q3FRAME Number Ice-like Ox: " << crystal.size() << " Number Liquid-like Ox: " << liquid.size() << endl;
...
void q3_r::qlloxygens(const vector<int> &crystal, const float* coord_norm, const float topInterface, const float botInterface){

	//Determine if molecules are in top or bottom QLL
	for (int i=0;i<num_ox;i++) {
		if ( coord_norm[i*3] > topInterface) {qllTop.push_back(i);}
		else if ( coord_norm[i*3] < botInterface) {qllBot.push_back(i);}
	}
	...
	for (int i=0;i<liquid.size();i++) {
		if ( coord_norm[liquid[i]*3] > topInterface || coord_norm[liquid[i]*3] < botInterface) { liq_count=liq_count+1;}
		if ( coord_norm[liquid[i]*3] > topInterface) { liqTop_count=liqTop_count+1;}
	}
	...
	cout << "#QLLOXYGENS number top layer: " << qllTop.size() << " bottom layer: " << qllBot.size() << endl;


	cout << "#QLLOXYGENS Number Ice-like Oxygens: " << crystal.size() << " Number QLL Ice-like Oxygens:   " << qllCrystalOx.size() << " Number TOP QLL Ice-like Oxygens: " << qllCrystalTop.size() << endl;
	cout << "#QLLOXYGENS Number liquid-like Oxygens:  " << liquid.size() <<  " Number QLL liquid-like Oxygens: " << liq_count << " Number TOP QLL liquid-like Oxygens: " << liqTop_count << endl;
...
q3_r::~q3_r() {
	q3f.close();	//do this at the end once read in every frame
}
