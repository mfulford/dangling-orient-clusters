...
//need to define it once for global use. cell dimensions from cell data
float pbc_x;
float pbc_y;
float pbc_z;
...
vector<bool> pbc_check::Phase() {
	vector<bool> phase_bool(3);
	phase_bool[0] = false;
	phase_bool[1] = false;
	phase_bool[2] = false;

	if ((pbc_x > pbc_y) && (pbc_x > pbc_z)) {
		cout << "X-axis: Secondary Prism Surface" << endl;
		phase_bool[0] = true;
	}
	else if ((pbc_y > pbc_x)&& (pbc_y > pbc_z)) {
		cout << "Y-axis: Prism Surface" << endl;
		phase_bool[1] = true;
	}
	else if ((pbc_z > pbc_x)&& (pbc_z > pbc_y)) {
		cout << "Z-axis: Basal Surface" << endl;
		phase_bool[2] = true;
	}
	return phase_bool;
}
...
float pbc_check::box_centre(int NATOM,const float* coord_norm) {
	...
	box_middle= *min_element(coord_norm,coord_norm+NATOM) +
			( *max_element(coord_norm,coord_norm+NATOM) - *min_element(coord_norm,coord_norm+NATOM) )/2.0;
	cout << "Box centre: " << box_middle << endl;
	return box_middle;
}

// Average theta (orientation) and q3 of dangling molecules:
void Stats::average(const vector< vector<int> > &DanglingTop, const vector< vector<int> > &DanglingBot, const vector< vector<float> > &orient, const vector<float> &q3or) {
	...
	cout << "#ORIENTDANG Top Layer Average: " << avOrient_Top << " Bottom Layer: " << avOrient_Bot << endl;
	cout << "#Q3DANG Top Layer Average: " << avQ3_Top << " Bottom Layer: " << avQ3_Bot << endl;

}

// Average q3 and orientation of QLL molecules:
void Stats::average(const vector<int> &QLLTop, const vector<int> &QLLBot, const vector< vector<float> > &orient, const vector<float> &q3or) {
	...
	for (int i=0;i<QLLTop.size();i++) {avOrient_Top = orient[0][QLLTop[i]] + avOrient_Top;}
	for (int i=0;i<QLLBot.size();i++) {avOrient_Bot = orient[0][QLLBot[i]] + avOrient_Bot;}

	for (int i=0;i<QLLTop.size();i++) {avQ3_Top = q3or[QLLTop[i]] + avQ3_Top;}
	for (int i=0;i<QLLBot.size();i++) {avQ3_Bot = q3or[QLLBot[i]] + avQ3_Bot;}
	...
	cout << "#ORIENTQLL Top Layer Average: " << avOrient_Top << " Bottom Layer: " << avOrient_Bot << endl;
	cout << "#Q3QLL Top Layer Average: " << avQ3_Top << " Bottom Layer: " << avQ3_Bot << endl;
...
float pbc_check::getdistx() const {return distx_sqr;}
float pbc_check::getdisty() const {return disty_sqr;}
float pbc_check::getdistz() const {return distz_sqr;}
...
vector<float> pbc_check::h_midpoint(const float x1, const float y1, const float z1, const float x2, const float y2, const float z2){
	...
	if (x_dist >= pbc_x/2) {x_mid = x_mid + (pbc_x/2.0);}
	if (y_dist >= pbc_y/2) {y_mid = y_mid + (pbc_y/2.0);}
	if (z_dist >= pbc_z/2) {z_mid = z_mid + (pbc_z/2.0);} 
