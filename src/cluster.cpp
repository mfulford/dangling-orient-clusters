		...
		for (int j=0;j< GlobalConnectVec[OxygenPhase[i]].size();j++) {
			...
			for (int k=0;k<phase_size;k++) {
				...
				if ( GlobConnect_id == OxygenPhase[k]) {
					if (OxygenPhase[i] == OxygenPhase[k]) { cout << "####CLUSTERconnect ERROR in Connectivity: i and k " << i << " " << k << endl;}
...
// read in LiquidConnectVec (matrix connectivity)  and liquid (which tells Ox id of row number in LiquidConnectVec)
void cluster::size(const float* coord_norm, vector<bool> IcePhase) {
	...
	Max = size[0];
	SecMax = size[1];
	cout << "#CLUSTERsize Largest cluster size: " << Max << " Second Largest: " << SecMax << endl;
	cout << "#CLUSTERsize Sum of All cluster sizes: " << accumulate(size.begin(),size.end(),0)  << endl;	
	...
	for (int i=0;i<clusterId.size();i++) {
		...
		for (int j=0;j<ClusterSize;j++) {
			ClusterCoordVec.push_back(coord_norm[clusterId[i][j]*3]); /*push back y coordinate of Oxygen members of Max cluster */
		}

		int num_atoms;
		float cutoff;
		if (ClusterSize < 2000) {
			minCoord = *min_element(ClusterCoordVec.begin(), ClusterCoordVec.end());
			maxCoord = *max_element(ClusterCoordVec.begin(), ClusterCoordVec.end());
		}
		else {
			...
			sort(ClusterCoordVec.begin(), ClusterCoordVec.end(), greater<float>());
			...
			if ( ClusterSize >=2000) {
				if (IcePhase[0] == true) {
					num_atoms = 110; cutoff =1.8; //X-surf Secondary Prism
				}
				else {
					num_atoms=130; cutoff =1.8; // Prism/Basal Y/Z-surf
				}
			...
			for (int ii=i_top;ii<i_bot;ii++){maxCoord = ClusterCoordVec[ii] + maxCoord;}
			...
			if ( ClusterSize >=2000) {
				if (IcePhase[0] == true) {num_atoms = 110; cutoff =1.8;} // X-surf Secondary Prism
				else {num_atoms=130; cutoff =1.8;} // Y/Z-surf: Prism/Basal
			...
			for (int ii=i_top;ii<i_bot;ii++){minCoord = ClusterCoordVec[ii] + minCoord;}
	...
	sort(TopCluster_info.begin(), TopCluster_info.end(), [](const vector< int >& a, const vector< int >& b){ return a[1] > b[1]; } );
	sort(BotCluster_info.begin(), BotCluster_info.end(), [](const vector< int >& a, const vector< int >& b){ return a[1] > b[1]; } );
