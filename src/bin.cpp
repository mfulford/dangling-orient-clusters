...
float bin::Initialise(bool FirstFrame, int NOX, const float* coord_norm) {
	float ox_coord[NOX];    
	...
	if (FirstFrame) {
		...
		partial_sort(ox_tmp.begin(), ox_tmp.begin()+20, ox_tmp.end(), greater<float>());
		int tmp_id = 0;
		...
		partial_sort(ox_tmp.begin(), ox_tmp.begin()+20, ox_tmp.end(), less<float>());
		tmp_id = 0;
		...
		cout << "#BinInfo. maxbin: " << maxbin << " minbin: " << minbin << " range: " << range << " bindiwth: " << binwidth << endl;
	}
...
void bin::Average(const std::vector<float> &q3or, const vector<vector<int> > &binVec) {
...
void bin::Average(const std::vector< std::vector<float> > &orient, const vector<vector<int> > &binVec) {
...
void bin::TimeAverage(int nqllTop, bool FirstFrame, vector<bool> IcePhase, const vector<int> &NumElementsVec) {

	width_qll;  						// width of nQLL range
	if (IcePhase[0] == true) { 			// X-surf: Secondary Prism
		width_qll=288;
	}
	else if (IcePhase[1] == true) { 	// Y-surf: Prism
		width_qll=240;
	}
	else if (IcePhase[2] == true) { 	// Z-surf: Basal
		width_qll=240;
	}
	...
	// Determine what nqll bin this frame belongs to. Floor rounds down so e.g. n=77 is bin 0 and 300 is bin 1
	qllbinID=floor(nqllTop/width_qll);

	// Check if particular nqllbin has been visited. If not: initialise
	if (count_nQLL[qllbinID] == 0) {
		...
		for (int i=0;i<nbins;i++) {
			...
			tot_NumElementsVec_nqll[qllbinID][i] = tot_NumElementsVec_nqll[qllbinID][i] + NumElementsVec[i];
		}
	}
...
void bin::orient_slice(int nqllTop, const vector< vector<int> > &slices, const vector< vector<float> > &orient, const float* coord_inplane, const float* z, bool IsConstQLL, bool FirstFrame) {
	if (FirstFrame) {n_slice_bins =100;}
	float or0;
	if(IsConstQLL) {
		if (FirstFrame) {
		...
		for (int s=0;s<num_slices;s++) {
			for (int i=0;i<slices[s].size();i++) {
				or0 = floor((((orient[0][slices[s][i]])*n_slice_bins)+ n_slice_bins)/2); // range -1 to 1 --> bin 0 to nbins-1 (0 to 99))
				if (or0 == n_slice_bins) { or0 = or0 -1;}
		...
		for (int s=0;s<num_slices;s++) {
			for (int i=0;i<slices[s].size();i++) {
				or0 = floor((((orient[0][slices[s][i]])*n_slice_bins)+ n_slice_bins)/2); // range -1 to 1 --> bin 0 to nbins-1 (0 to 99))
				if (or0 == n_slice_bins) { or0 = or0 -1;}
				OrientSlice_nqll[qllbinID][s][or0]= OrientSlice_nqll[qllbinID][s][or0] +1;
			}
		}
	}
	...
	if (IsConstQLL) {
		...
		for (int n=0;n<nqllbins;n++) {
			if (count_nQLL[n] > 0) {
				for (int s=0;s<num_slices;s++) {
					for (int b=0;b<n_slice_bins;b++) {
						OrientSlice_nqll[n][s][b]= OrientSlice_nqll[n][s][b]/countSlice_nqll[n][s];
					}
				}
			}
		}
...
const vector<float> bin::getTimeBinAvVec(bool IsDensity) {
	...
	if (IsDensity) { 	
		...
		for (int i=0;i<nbins;i++){ totalnum_elements = tot_NumElementsVec[i] + totalnum_elements;}	
		for (int i=0;i<nbins;i++){ TimeBinAvVec[i] = TimeBinAvVec[i]/totalnum_elements;}
	}
	else {
		for (int i=0;i<nbins;i++){
			if (tot_NumElementsVec[i] > 0) {TimeBinAvVec[i] = TimeBinAvVec[i]/tot_NumElementsVec[i];}
		}
	}
	return TimeBinAvVec;
}
	...
	for (int n=0;n<nqllbins;n++) {
		...
		if (count_nQLL[n] > 0) {
			...
			if (IsDensity) {
				int totalnum_elements=0;
				for (int i=0;i<nbins;i++){
					totalnum_elements = tot_NumElementsVec_nqll[n][i] + totalnum_elements;
				}
				for (int i=0;i<nbins;i++) {
				...
				for (int i=0;i<nbins;i++) {
					if (tot_NumElementsVec_nqll[n][i] > 0) {
						TimeAv_nQLL[n][i] = TimeAv_nQLL[n][i]/tot_NumElementsVec_nqll[n][i];
					}
				}
			}
		}
 ...
 */
void bin::InPlaneDensity(int NOX, const vector< vector<int> > &slices, const vector< vector<float> > &orient, const vector<float> &q3or, const float* coord_inplane, const float* z) {

	int n_zbins=45;
	int n_planebins=45;
	int n_orientbins=100;

	num_slices = slices.size(); // instead of all slices maybe use top/bottom 10
	...
	for (int s=0;s<num_slices;s++) {
		for (int i=0;i<slices[s].size();i++) {
			...
			if (bin_theta == n_orientbins) { 			// occurs if rho or theta = 1
			...
			int ID_planebin = floor(n_planebins*(tmp_plane - minPlane)/rangePlane);
			if (ID_zbin == n_zbins) {
				ID_zbin = n_zbins -1;
			}
			...
			if ((ID_zbin >= n_zbins) || (ID_planebin >=n_zbins)) { cout << "ERRORR " << ID_zbin << " " << ID_planebin << endl; }

			orientPlaneVec[s][bin_theta][bin_rho] = orientPlaneVec[s][bin_theta][bin_rho] + 1.00;
			PlaneVec[s][0][ID_zbin][ID_planebin]  = PlaneVec[s][0][ID_zbin][ID_planebin]  + 1.00;
			PlaneVec[s][1][ID_zbin][ID_planebin]  = PlaneVec[s][1][ID_zbin][ID_planebin]  + q3or[slices[s][i]];      // <q3> slice
			PlaneVec[s][2][ID_zbin][ID_planebin]  = PlaneVec[s][2][ID_zbin][ID_planebin]  + orient[0][slices[s][i]]; // cos(theta).
...
void bin::InPlaneTimeAv(int nqllTop, bool IsConstQLL, bool FirstFrame, const vector< vector<int> > &slices) {
	...
	if (IsConstQLL) {
			...
			for (int layer=0;layer<num_slices;layer++) {	
				...
				for (int i=0;i<100;i++) { // cos(theta)
					for (int j=0;j<100;j++) { //rho
						avOrientPlaneVec[layer][i][j] = avOrientPlaneVec[layer][i][j] + orientPlaneVec[layer][i][j];
					}
				}
				...
				for (int param=0;param<3;param++) {
					...
					for (int i=0;i<n_z;i++) {
						for (int j=0;j<n_plane;j++) {
							avPlaneVec[layer][param][i][j] = avPlaneVec[layer][param][i][j] + PlaneVec[layer][param][i][j]; //Layer 0=top, 1=bot; param 1,2,3 = dens,q3,orient
		...
		if (Unvisited_nQLL[qllbinID]) {
			...
			for (int layer=0;layer<num_slices;layer++) {	// was 2
				countSlice_nqll[qllbinID][layer] = countSlice_nqll[qllbinID][layer] + slices[layer].size();

				for (int i=0;i<100;i++) { // cos(theta)
					for (int j=0;j<100;j++) { //rho
						OrientAvPlane_nQLL[qllbinID][layer][i][j] = OrientAvPlane_nQLL[qllbinID][layer][i][j] + orientPlaneVec[layer][i][j];
					}
				}
				...
				for (int param=0;param<3;param++) {
					for (int z_grid=0;z_grid<n_z;z_grid++) {
						for (int pl_grid=0;pl_grid<n_plane;pl_grid++) {
							AvPlane_nQLL[qllbinID][layer][param][z_grid][pl_grid] = PlaneVec[layer][param][z_grid][pl_grid] + AvPlane_nQLL[qllbinID][layer][param][z_grid][pl_grid];
						}
					}
				}
			} // layers loop
...
void bin::NormaliseHeatmaps() {
			...
			for (int i=0;i<n_z;i++) {
				for (int j=0;j<n_plane;j++) {
					if (avPlaneVec[layer][0][i][j] > 0) {
						avPlaneVec[layer][param][i][j] = avPlaneVec[layer][param][i][j]/avPlaneVec[layer][0][i][j];
					}
					if (param == 2) { // can normalise density as don't need it anymore for q3 or orient
						avPlaneVec[layer][0][i][j] = avPlaneVec[layer][0][i][j]/countSlice[layer];
					}
				}
			}
		}
	...
	for (int layer=0;layer<num_slices;layer++) {
		for (int i=0;i<100;i++) { //costheta
			for (int j=0;j<100;j++) { // rho
				avOrientPlaneVec[layer][i][j] = avOrientPlaneVec[layer][i][j]/countSlice[layer];
			}
		}
	}
}

const vector<vector<vector<float> > > bin::getAvOrientPlaneVec() {return avOrientPlaneVec;}

const vector<vector<vector<vector<float> > > > bin::getAvPlaneVec() {return avPlaneVec;}
...
void bin::NormaliseHeatmaps_nQLL() {
	for (int n=0;n<nqllbins;n++) {
		if (count_nQLL[n] > 0) {
			...
			cout << "Check Vec " << count_nQLL[n] << " " << n_z << " " << n_plane << endl;
			for (int layer=0;layer<num_slices;layer++) { //was 2
				for (int param=1;param<3;param++) {
					for (int i=0;i<n_z;i++) {
						for (int j=0;j<n_plane;j++) {
							if (AvPlane_nQLL[n][layer][0][i][j] > 0) {
								AvPlane_nQLL[n][layer][param][i][j] = AvPlane_nQLL[n][layer][param][i][j]/AvPlane_nQLL[n][layer][0][i][j];
							}
							if (param == 2) {
								AvPlane_nQLL[n][layer][0][i][j] = AvPlane_nQLL[n][layer][0][i][j]/countSlice_nqll[n][layer];
							}
				...
				for (int i=0;i<100;i++) { //costheta
					for (int j=0;j<100;j++) { // rho
						OrientAvPlane_nQLL[n][layer][i][j] = OrientAvPlane_nQLL[n][layer][i][j]/countSlice_nqll[n][layer];
					}
				}
...
const vector<vector<vector<vector<vector<float> > > > > bin::getAvPlane_nQLL() {return  AvPlane_nQLL;}
// Return heatmap dynamic QLL
const vector<vector<vector<vector<float> > > > bin::getOrientAvPlane_nQLL() {return OrientAvPlane_nQLL;}
