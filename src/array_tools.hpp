...
//---------------------------------
public:
	ARRAY_3D(unsigned int _d1, unsigned int _d2, unsigned int _d3) :
...
//         T = nullptr;
		...
		try {
			...
			std::cerr
					<< "Error while allocating internal memory for an ARRAY_3D : "
					<< e.what() << std::endl;
		}
	...
	void dump() {
		for (unsigned int i = 0; i < siz; i++)
			std::cout << "Dump of value at rank " << i << " : " << ptr[i]
					<< std::endl;
	}
...
//---------------------------------
public:
	ARRAY_2D(unsigned int _d1, unsigned int _d2) :
			d1(_d1), d2(_d2) {
//         T = nullptr;
		siz = d1 * d2;
		...
		try {
			...
			std::cerr
					<< "Error while allocating internal memory for an ARRAY_2D : "
					<< e.what() << std::endl;
		}
